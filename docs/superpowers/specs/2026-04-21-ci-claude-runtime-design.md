...
5. ~~**Marketplace sha bump cadence.**~~ **Resolved 2026-05-05.** Bump cadence is **manual on observed value**. Every PR that bumps `sources.marketplace.ref` MUST include in the body a `git diff` of the marketplace repo between the old and new SHA, scoped to the plugin paths that appear in the manifest. Automation is intentionally absent — marketplace upstream changes (agent renames, hook schema shifts, package layout changes) require human review before promotion, and that review is the load-bearing gate, not a mechanical pin-bump. Per spec §10.2 ("Marketplace bump review containment"). A comment block at the top of `runtime/ci-manifest.yaml` near `sources.marketplace.ref` codifies this policy inline; that comment lands in the Phase 6 PR (#144). The decision predates this entry — Phase 5 (#188) shipped a manual pin without automation by default; this entry simply makes the policy explicit so future contributors don't propose drift-detection automation that would weaken the human-review requirement.
