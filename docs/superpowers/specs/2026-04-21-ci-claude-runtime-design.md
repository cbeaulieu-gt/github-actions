...
5. ~~**Marketplace sha bump cadence.**~~ **Resolved 2026-05-05.** Bump cadence is **manual on observed value**. Every PR that bumps `sources.marketplace.ref` MUST include in the body a `git diff` of the marketplace repo between old and new SHA, scoped to plugin paths in the manifest. Per §10.2 ("Marketplace bump review containment"). Automation is intentionally absent — marketplace upstream changes (agent renames, hook schema shifts) require human review before promotion. Phase 5 (#188) implemented this policy implicitly; this entry formalizes the decision so future contributors don't propose drift-detection automation that would weaken the human-review gate. A comment block in `runtime/ci-manifest.yaml` near `sources.marketplace.ref` lands in Phase 6 (#144).
