...
**`pull_request_target` actions that read PR file content must explicitly check out the head SHA.** The default `actions/checkout@v5` on `pull_request_target` resolves to the base ref. Any downstream step that runs `git hash-object`, inventories changed files, or otherwise reads PR file content from disk will see only base-ref state and fail. Add `ref: ${{ github.event.pull_request.head.sha }}` to the checkout — but **only after auditing that no pre-authz step executes content from the head ref**. See `pr-review/action.yml` for the canonical pattern (security analysis in the inline comment). Background: #197.
