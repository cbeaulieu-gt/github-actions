...
**Container git safety (post-#199):** the runtime base image bakes `git config --system --add safe.directory '*'` into `/etc/gitconfig`. This prevents `git: detected dubious ownership` failures in any composite action that invokes git against the actions/checkout-written workspace from inside the digest-pinned container — root cause of #197 and the original blocker for `claude-code-action@v1`'s `track_progress` setup. STAGE 4 overlay smoke (`runtime/scripts/smoke-test.sh` section d.5) asserts the exemption is present in every overlay; future Dockerfile edits that shadow `/etc/gitconfig` (e.g. with a custom `--config-file` directive) would trip the check.
